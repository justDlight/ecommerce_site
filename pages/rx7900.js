...
import products21 from '../products21.json';

export default function Home() {
  const [searchTerm, setSearchTerm] = useState('');

  const filteredProducts = products21.filter(product =>
          ...
          <div className="row">
            ...
            <div className="col" id="product-col">
              <div className={styles.grid}>
                {filteredProducts.map(products21 => {
                  return (
                    <div key={products21.id} className={styles.card}>
                      <img src={products21.image} alt={`Preview of ${products21.title}`} />
                      <h3 id="product-title">
                        <a href={`/${products21.title}`}>{ products21.title }</a>
                      </h3>
                      <p id="product-description">{ products21.description }</p>
                      <p id="product-price">${ products21.price }</p>
                      <p>
                        <button id="test_id" className="snipcart-add-item"
                          data-item-id={products21.id}
                          data-item-image={products21.image}
                          data-item-name={products21.title}
                          data-item-url="/"
                          data-item-price={products21.price}
                        >
                          Add to Cart
                        </button>
                      </p>
