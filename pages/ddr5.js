...
import products26 from '../products26.json';

export default function Home() {
  const [searchTerm, setSearchTerm] = useState('');

  const filteredProducts = products26.filter(product =>
          ...
          <div className="row">
            ...
            <div className="col" id="product-col">
              <div className={styles.grid}>
                {filteredProducts.map(products26 => {
                  return (
                    <div key={products26.id} className={styles.card}>
                      <img src={products26.image} alt={`Preview of ${products26.title}`} />
                      <h3 id="product-title">
                        <a href={`/${products26.title}`}>{ products26.title }</a>
                      </h3>
                      <p id="product-description">{ products26.description }</p>
                      <p id="product-price">${ products26.price }</p>
                      <p>
                        <button id="test_id" className="snipcart-add-item"
                          data-item-id={products26.id}
                          data-item-image={products26.image}
                          data-item-name={products26.title}
                          data-item-url="/"
                          data-item-price={products26.price}
                        >
                          Add to Cart
                        </button>
                      </p>
