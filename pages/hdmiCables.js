...
import products8 from '../products8.json';

export default function Home() {
  const [searchTerm, setSearchTerm] = useState('');

  const filteredProducts = products8.filter(product =>
          ...
          <div className="row">
            ...
            <div className="col" id="product-col">
              <div className={styles.grid}>
                {filteredProducts.map(products8 => {
                  return (
                    <div key={products8.id} className={styles.card}>
                      <img src={products8.image} alt={`Preview of ${products8.title}`} />
                      <h3 id="product-title">
                        <a href={`/${products8.title}`}>{ products8.title }</a>
                      </h3>
                      <p id="product-description">{ products8.description }</p>
                      <p id="product-price">${ products8.price }</p>
                      <p>
                        <button id="test_id" className="snipcart-add-item"
                          data-item-id={products8.id}
                          data-item-image={products8.image}
                          data-item-name={products8.title}
                          data-item-url="/"
                          data-item-price={products8.price}
                        >
                          Add to Cart
                        </button>
                      </p>
