...
import products27 from '../products27.json';

export default function Home() {
  const [searchTerm, setSearchTerm] = useState('');

  const filteredProducts = products27.filter(product =>
          ...
          <div className="row">
            ...
            <div className="col" id="product-col">
              <div className={styles.grid}>
                {filteredProducts.map(products27 => {
                  return (
                    <div key={products27.id} className={styles.card}>
                      <img src={products27.image} alt={`Preview of ${products27.title}`} />
                      <h3 id="product-title">
                        <a href={`/${products27.title}`}>{ products27.title }</a>
                      </h3>
                      <p id="product-description">{ products27.description }</p>
                      <p id="product-price">${ products27.price }</p>
                      <p>
                        <button id="test_id" className="snipcart-add-item"
                          data-item-id={products27.id}
                          data-item-image={products27.image}
                          data-item-name={products27.title}
                          data-item-url="/"
                          data-item-price={products27.price}
                        >
                          Add to Cart
                        </button>
                      </p>
