...
import products11 from '../products11.json';

export default function Home() {
  const [searchTerm, setSearchTerm] = useState('');

  const filteredProducts = products11.filter(product =>
          ...
          <div className="row">
            ...
            <div className="col" id="product-col">
              <div className={styles.grid}>
                {filteredProducts.map(products11 => {
                  return (
                    <div key={products11.id} className={styles.card}>
                      <img src={products11.image} alt={`Preview of ${products11.title}`} />
                      <h3 id="product-title">
                        <a href={`/${products11.title}`}>{ products11.title }</a>
                      </h3>
                      <p id="product-description">{ products11.description }</p>
                      <p id="product-price">${ products11.price }</p>
                      <p>
                        <button id="test_id" className="snipcart-add-item"
                          data-item-id={products11.id}
                          data-item-image={products11.image}
                          data-item-name={products11.title}
                          data-item-url="/"
                          data-item-price={products11.price}
                        >
                          Add to Cart
                        </button>
                      </p>
