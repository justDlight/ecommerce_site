...
import products12 from '../products12.json';

export default function Home() {
  const [searchTerm, setSearchTerm] = useState('');

  const filteredProducts = products12.filter(product =>
          ...
          <div className="row">
            ...
            <div className="col" id="product-col">
              <div className={styles.grid}>
                {filteredProducts.map(products12 => {
                  return (
                    <div key={products12.id} className={styles.card}>
                      <img src={products12.image} alt={`Preview of ${products12.title}`} />
                      <h3 id="product-title">
                        <a href={`/${products12.title}`}>{ products12.title }</a>
                      </h3>
                      <p id="product-description">{ products12.description }</p>
                      <p id="product-price">${ products12.price }</p>
                      <p>
                        <button id="test_id" className="snipcart-add-item"
                          data-item-id={products12.id}
                          data-item-image={products12.image}
                          data-item-name={products12.title}
                          data-item-url="/"
                          data-item-price={products12.price}
                        >
                          Add to Cart
                        </button>
                      </p>
