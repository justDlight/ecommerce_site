...
import products31 from '../products31.json';

export default function Home() {
  const [searchTerm, setSearchTerm] = useState('');

  const filteredProducts = products31.filter(product =>
          ...
          <div className="row">
            ...
            <div className="col" id="product-col">
              <div className={styles.grid}>
                {filteredProducts.map(products31 => {
                  return (
                    <div key={products31.id} className={styles.card}>
                      <img src={products31.image} alt={`Preview of ${products31.title}`} />
                      <h3 id="product-title">
                        <a href={`/${products31.title}`}>{ products31.title }</a>
                      </h3>
                      <p id="product-description">{ products31.description }</p>
                      <p id="product-price">${ products31.price }</p>
                      <p>
                        <button id="test_id" className="snipcart-add-item"
                          data-item-id={products31.id}
                          data-item-image={products31.image}
                          data-item-name={products31.title}
                          data-item-url="/"
                          data-item-price={products31.price}
                        >
                          Add to Cart
                        </button>
                      </p>
