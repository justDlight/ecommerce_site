...
import products30 from '../products30.json';

export default function Home() {
  const [searchTerm, setSearchTerm] = useState('');

  const filteredProducts = products30.filter(product =>
          ...
          <div className="row">
            ...
            <div className="col" id="product-col">
              <div className={styles.grid}>
                {filteredProducts.map(products30 => {
                  return (
                    <div key={products30.id} className={styles.card}>
                      <img src={products30.image} alt={`Preview of ${products30.title}`} />
                      <h3 id="product-title">
                        <a href={`/${products30.title}`}>{ products30.title }</a>
                      </h3>
                      <p id="product-description">{ products30.description }</p>
                      <p id="product-price">${ products30.price }</p>
                      <p>
                        <button id="test_id" className="snipcart-add-item"
                          data-item-id={products30.id}
                          data-item-image={products30.image}
                          data-item-name={products30.title}
                          data-item-url="/"
                          data-item-price={products30.price}
                        >
                          Add to Cart
                        </button>
                      </p>
