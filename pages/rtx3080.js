...
import products17 from '../products17.json';

export default function Home() {
  const [searchTerm, setSearchTerm] = useState('');

  const filteredProducts = products17.filter(product =>
          ...
          <div className="row">
            ...
            <div className="col" id="product-col">
              <div className={styles.grid}>
                {filteredProducts.map(products17 => {
                  return (
                    <div key={products17.id} className={styles.card}>
                      <img src={products17.image} alt={`Preview of ${products17.title}`} />
                      <h3 id="product-title">
                        <a href={`/${products17.title}`}>{ products17.title }</a>
                      </h3>
                      <p id="product-description">{ products17.description }</p>
                      <p id="product-price">${ products17.price }</p>
                      <p>
                        <button id="test_id" className="snipcart-add-item"
                          data-item-id={products17.id}
                          data-item-image={products17.image}
                          data-item-name={products17.title}
                          data-item-url="/"
                          data-item-price={products17.price}
                        >
                          Add to Cart
                        </button>
                      </p>
