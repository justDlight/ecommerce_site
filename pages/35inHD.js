...
import products22 from '../products22.json';

export default function Home() {
  const [searchTerm, setSearchTerm] = useState('');

  const filteredProducts = products22.filter(product =>
          ...
          <div className="row">
            ...
            <div className="col" id="product-col">
              <div className={styles.grid}>
                {filteredProducts.map(products22 => {
                  return (
                    <div key={products22.id} className={styles.card}>
                      <img src={products22.image} alt={`Preview of ${products22.title}`} />
                      <h3 id="product-title">
                        <a href={`/${products22.title}`}>{ products22.title }</a>
                      </h3>
                      <p id="product-description">{ products22.description }</p>
                      <p id="product-price">${ products22.price }</p>
                      <p>
                        <button id="test_id" className="snipcart-add-item"
                          data-item-id={products22.id}
                          data-item-image={products22.image}
                          data-item-name={products22.title}
                          data-item-url="/"
                          data-item-price={products22.price}
                        >
                          Add to Cart
                        </button>
                      </p>
