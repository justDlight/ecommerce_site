...
import products19 from '../products19.json';

export default function Home() {
  const [searchTerm, setSearchTerm] = useState('');

  const filteredProducts = products19.filter(product =>
          ...
          <div className="row">
            ...
            <div className="col" id="product-col">
              <div className={styles.grid}>
                {filteredProducts.map(products19 => {
                  return (
                    <div key={products19.id} className={styles.card}>
                      <img src={products19.image} alt={`Preview of ${products19.title}`} />
                      <h3 id="product-title">
                        <a href={`/${products19.title}`}>{ products19.title }</a>
                      </h3>
                      <p id="product-description">{ products19.description }</p>
                      <p id="product-price">${ products19.price }</p>
                      <p>
                        <button id="test_id" className="snipcart-add-item"
                          data-item-id={products19.id}
                          data-item-image={products19.image}
                          data-item-name={products19.title}
                          data-item-url="/"
                          data-item-price={products19.price}
                        >
                          Add to Cart
                        </button>
                      </p>
