...
import products15 from '../products15.json';

export default function Home() {
  const [searchTerm, setSearchTerm] = useState('');

  const filteredProducts = products15.filter(product =>
          ...
          <div className="row">
            ...
            <div className="col" id="product-col">
              <div className={styles.grid}>
                {filteredProducts.map(products15 => {
                  return (
                    <div key={products15.id} className={styles.card}>
                      <img src={products15.image} alt={`Preview of ${products15.title}`} />
                      <h3 id="product-title">
                        <a href={`/${products15.title}`}>{ products15.title }</a>
                      </h3>
                      <p id="product-description">{ products15.description }</p>
                      <p id="product-price">${ products15.price }</p>
                      <p>
                        <button id="test_id" className="snipcart-add-item"
                          data-item-id={products15.id}
                          data-item-image={products15.image}
                          data-item-name={products15.title}
                          data-item-url="/"
                          data-item-price={products15.price}
                        >
                          Add to Cart
                        </button>
                      </p>
