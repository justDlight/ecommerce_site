...
import products13 from '../products13.json';

export default function Home() {
  const [searchTerm, setSearchTerm] = useState('');

  const filteredProducts = products13.filter(product =>
          ...
          <div className="row">
            ...
            <div className="col" id="product-col">
              <div className={styles.grid}>
                {filteredProducts.map(products13 => {
                  return (
                    <div key={products13.id} className={styles.card}>
                      <img src={products13.image} alt={`Preview of ${products13.title}`} />
                      <h3 id="product-title">
                        <a href={`/${products13.title}`}>{ products13.title }</a>
                      </h3>
                      <p id="product-description">{ products13.description }</p>
                      <p id="product-price">${ products13.price }</p>
                      <p>
                        <button id="test_id" className="snipcart-add-item"
                          data-item-id={products13.id}
                          data-item-image={products13.image}
                          data-item-name={products13.title}
                          data-item-url="/"
                          data-item-price={products13.price}
                        >
                          Add to Cart
                        </button>
                      </p>
