...
import products7 from '../products7.json';

export default function Home() {
  const [searchTerm, setSearchTerm] = useState('');

  const filteredProducts = products7.filter(product =>
          ...
          <div className="row">
            ...
            <div className="col" id="product-col">
              <div className={styles.grid}>
                {filteredProducts.map(products7 => {
                  return (
                    <div key={products7.id} className={styles.card}>
                      <img src={products7.image} alt={`Preview of ${products7.title}`} />
                      <h3 id="product-title">
                        <a href={`/${products7.title}`}>{ products7.title }</a>
                      </h3>
                      <p id="product-description">{ products7.description }</p>
                      <p id="product-price">${ products7.price }</p>
                      <p>
                        <button id="test_id" className="snipcart-add-item"
                          data-item-id={products7.id}
                          data-item-image={products7.image}
                          data-item-name={products7.title}
                          data-item-url="/"
                          data-item-price={products7.price}
                        >
                          Add to Cart
                        </button>
                      </p>
