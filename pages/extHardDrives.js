...
import products25 from '../products25.json';

export default function Home() {
  const [searchTerm, setSearchTerm] = useState('');

  const filteredProducts = products25.filter(product =>
          ...
          <div className="row">
            ...
            <div className="col" id="product-col">
              <div className={styles.grid}>
                {filteredProducts.map(products25 => {
                  return (
                    <div key={products25.id} className={styles.card}>
                      <img src={products25.image} alt={`Preview of ${products25.title}`} />
                      <h3 id="product-title">
                        <a href={`/${products25.title}`}>{ products25.title }</a>
                      </h3>
                      <p id="product-description">{ products25.description }</p>
                      <p id="product-price">${ products25.price }</p>
                      <p>
                        <button id="test_id" className="snipcart-add-item"
                          data-item-id={products25.id}
                          data-item-image={products25.image}
                          data-item-name={products25.title}
                          data-item-url="/"
                          data-item-price={products25.price}
                        >
                          Add to Cart
                        </button>
                      </p>
