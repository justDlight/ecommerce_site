...
import products9 from '../products9.json';

export default function Home() {
  const [searchTerm, setSearchTerm] = useState('');

  const filteredProducts = products9.filter(product =>
          ...
          <div className="row">
            ...
            <div className="col" id="product-col">
              <div className={styles.grid}>
                {filteredProducts.map(products9 => {
                  return (
                    <div key={products9.id} className={styles.card}>
                      <img src={products9.image} alt={`Preview of ${products9.title}`} />
                      <h3 id="product-title">
                        <a href={`/${products9.title}`}>{ products9.title }</a>
                      </h3>
                      <p id="product-description">{ products9.description }</p>
                      <p id="product-price">${ products9.price }</p>
                      <p>
                        <button id="test_id" className="snipcart-add-item"
                          data-item-id={products9.id}
                          data-item-image={products9.image}
                          data-item-name={products9.title}
                          data-item-url="/"
                          data-item-price={products9.price}
                        >
                          Add to Cart
                        </button>
                      </p>
